...
let lastUpdated = new Date(books[0].date);

const router = new Router();
router.get('/books', ctx => {
  const ifModifiedSince = ctx.request.get('If-Modified-Since');
  if (ifModifiedSince && new Date(ifModifiedSince).getTime() < new Date(lastUpdated).getTime() - new Date(lastUpdated).getMilliseconds()) {
    ctx.response.status = 304;
  } else {
    ctx.response.set('Last-Modified', new Date(lastUpdated).toISOString());
